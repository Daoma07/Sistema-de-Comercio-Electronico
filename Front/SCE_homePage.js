...
function visualizarProductos() {
    ...
    for (let i = 0; i < productos.length; i++) {
        if (productos[i].existencia > 0) {
            ...
            contenedor.innerHTML += `<div><img src="${productos[i].urlImagen}"><div class="informacion"><p>${productos[i].nombre}</p><p class="precio">$${productos[i].valor}</p><p class="soldOut">Agotado</p></div></div>`
        }
    }
}
...
function mostrarElemtrosLista() {
    ...
    for (let i = 0; i < lista.length; i++){
        productosCompra.innerHTML += `<div><div class="img"><button onclick=eliminar(${i}) class="botonTrash"><img src="trashCan.png"></button><p>${lista[i].nombre}</p></div><p> $${lista[i].precio}</p></div>`
